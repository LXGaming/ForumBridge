...
public class PacketManager {
    ...
    public static void process(JsonObject jsonObject) {
        ...
        if (Toolbox.isBlank(packetId)) {
            Bridge.getInstance().getLogger().warn("Received invalid packet");
            return;
        }
        
        Class<? extends Packet> packetClass = getPacketClass(packetId).orElse(null);
        if (packetClass == null) {
            Bridge.getInstance().getLogger().warn("Received packet with unknown id {}", packetId);
            return;
        }
        
        Packet packet = Toolbox.parseJson(jsonObject.get("data"), packetClass).orElse(null);
        if (packet == null) {
            Bridge.getInstance().getLogger().warn("Failed to deserialize packet {}: {}", packetClass.getName(), Toolbox.GSON.toJson(jsonObject));
            return;
        }
        
        Bridge.getInstance().getLogger().debug("Processing {} ({}): {}", Toolbox.getClassSimpleName(packetClass), packetId, Toolbox.GSON.toJson(packet));
        for (NetworkHandler networkHandler : NETWORK_HANDLERS) {
            try {
                ...
                Bridge.getInstance().getLogger().error("Encountered an error processing {}::process", networkHandler.getClass().getName(), ex);
            }
        }
    }
    ...
    public static boolean registerNetworkHandler(Class<? extends NetworkHandler> networkHandlerClass) {
        if (NETWORK_HANDLER_CLASSES.contains(networkHandlerClass)) {
            Bridge.getInstance().getLogger().warn("{} is already registered", networkHandlerClass.getSimpleName());
            return false;
        }
        ...
        if (networkHandler == null) {
            Bridge.getInstance().getLogger().error("{} failed to initialize", networkHandlerClass.getSimpleName());
            return false;
        }
        
        NETWORK_HANDLERS.add(networkHandler);
        Bridge.getInstance().getLogger().debug("{} registered", networkHandlerClass.getSimpleName());
        return true;
    }
    
    public static boolean registerPacket(String id, Class<? extends Packet> packetClass) {
        if (PACKET_CLASSES.containsKey(id)) {
            Bridge.getInstance().getLogger().warn("{} is already registered", id);
            return false;
        }
        
        if (PACKET_CLASSES.containsValue(packetClass)) {
            Bridge.getInstance().getLogger().warn("{} is already registered", packetClass.getSimpleName());
            return false;
        }
